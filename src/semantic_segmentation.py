...
class SegnetSemantic:
    ...
    def __init__(self):
        ...
        self.rgb_frame = None
        self.height = rospy.get_param("rp_semantic/sensor_image_height", 480)
        self.width = rospy.get_param("rp_semantic/sensor_image_width", 640)

        self.publish_rgb_frame = rospy.get_param("rp_semantic/publish_rgb_frame", False)
        self.publish_rgb_labels = rospy.get_param("rp_semantic/publish_rgb_labels", False)
        ...
        self.semantic_rgb_frame_pub = rospy.Publisher('/rp_semantic/semantic_rgb_frame', Image, queue_size=1)
        self.rgb_frame_pub = rospy.Publisher('/rp_semantic/rgb_frame', Image, queue_size=1)
        ...
        caffe_root = rospy.get_param("rp_semantic/caffe_root_path", '')
        model_base_path = rospy.get_param("rp_semantic/caffe_model_path", '')
        ...
        colours_path = model_base_path + 'sun.png'
        ...
        if self.wait_for_segnet is False: # if main control loop not busy
            try:
                ...
                print 'obtain rgb frame'
                    ...
                    rospy.loginfo("segmentation_callback: Conversion from rosmsg to cv mat failed")
                    exit()
    ...
    def controlLoopIteration(self):
            ...
            segnet_prob_out = np.transpose(segnet_prob_out[1:, :, :], axes=(1,2,0))

            num_labels = segnet_prob_out.shape[2]

            segnet_reshaped_prob_out = np.zeros((self.height, self.width, num_labels))
            for cl in range(0, num_labels):
